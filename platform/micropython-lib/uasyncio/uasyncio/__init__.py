...
class PollEventLoop(EventLoop):
    ...
    def add_reader(self, sock, cb, *args):
        if DEBUG and __debug__:
            log.debug("add_reader%s", (id(sock), cb, args))
        if args:
        ...
        #
        self.sock_reg[id(sock)] = (sock, "Reader")

    def remove_reader(self, sock):
        if DEBUG and __debug__:
            log.debug("remove_reader(%s)", id(sock))
        self.objmap.pop(id(sock), None)
        self.poller.unregister(sock)

        self.sock_reg.pop(id(sock), None)

    def add_writer(self, sock, cb, *args):
        if DEBUG and __debug__:
            log.debug("add_writer%s", (id(sock), cb, args))
        if args:
        ...
        #
        self.sock_reg[id(sock)] = (sock, "Writer")

    def remove_writer(self, sock):
        if DEBUG and __debug__:
            log.debug("remove_writer(%s)", id(sock))
        self.objmap.pop(id(sock), None)
...
def start_server(client_coro, host, port, backlog=10):
    ...
    while True:
        if DEBUG and __debug__:
            log.debug("start_server: Before accept: Reader socket: {} - {}".format( id(s), ai ))
        yield IORead(s)
        if DEBUG and __debug__:
            log.debug("start_server: After iowait: Reader socket: {} - {}".format( id(s), ai ))
        s2, client_addr = s.accept()
        s2.setblocking(False)
        if DEBUG and __debug__:
            log.debug("start_server: After accept: %s - %s -", id(s2), client_addr)
